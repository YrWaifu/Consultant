...
def process_ad_check_task(text: str | None, media_path: str | None):
    ...
    try:
        ...
        if violations_count == 1:
            violations_text = f"Есть {violations_count} несоответствие ФЗ «О рекламе»"
        elif 2 <= violations_count <= 4:
            violations_text = f"Есть {violations_count} несоответствия ФЗ «О рекламе»"
        else:
            violations_text = f"Есть {violations_count} несоответствий ФЗ «О рекламе»"
        ...
        ring_deg = 360.0
        ring_label = "Нет" if has_violations else "Да"  # Исправлено: "Нет" если есть нарушения, "Да" если все в порядке
